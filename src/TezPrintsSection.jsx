          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div>
                ...
                <input value={form.category} onChange={e => set("category", e.target.value)} placeholder="e.g. Apparel, Print, Accessories" style={inputStyle} />
              </div>
              <div>
...
/* ── Product Card ── */
function ProductCard({ product, isAdmin, onAddCart, onEdit }) {
  ...
  return (
    ...
    >
      {discount > 0 && (
        <div style={{ position: "absolute", top: 10, left: 10, background: T.error, color: "#fff", borderRadius: 8, padding: "3px 8px", fontSize: 10, fontWeight: 800, zIndex: 1 }}>
          -{discount}%
        </div>
      )}
      {isAdmin && (
        <button onClick={() => onEdit(product)} style={{ position: "absolute", top: 10, right: 10, background: "#000a", border: "none", borderRadius: 8, padding: "5px 9px", color: "#fff", fontSize: 12, cursor: "pointer", zIndex: 1 }}>✏️</button>
      )}

      <div style={{ height: 160, background: T.bgInput, overflow: "hidden" }}>
      ...
      <div style={{ padding: "14px" }}>
        ...
        <div style={{ fontWeight: 700, fontSize: 13, color: T.text, marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", minHeight: 34 }}>{product.title}</div>
        ...
        {product.stock <= 5 && product.stock > 0 && (
          <div style={{ fontSize: 10, color: T.amber, fontWeight: 700, marginTop: 4 }}>Only {product.stock} left!</div>
        )}
        {product.stock === 0 && (
          <div style={{ fontSize: 10, color: T.error, fontWeight: 700, marginTop: 4 }}>Out of stock</div>
        )}
...
/* ── Main Section ── */
export default function TezPrintsSection({ session }) {
  ...
  if (!isAdmin && products.length === 0) return null;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <h3 style={{ fontWeight: 800, fontSize: 20, color: T.text }}>Official <span style={{ color: T.orange }}>Merchandise & Prints</span></h3>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {isAdmin && (
            <button onClick={() => setShowAdd(true)}
              style={{ background: T.orangeMd, border: `1px solid ${T.orange}44`, borderRadius: 10, padding: "9px 16px", color: T.orange, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              + Add Product
            </button>
          )}
          {!isAdmin && cart.length > 0 && (
            <button onClick={() => setShowCart(true)}
              ...
              🛒 Cart <span style={{ background: "#fff", color: T.orange, borderRadius: 20, padding: "1px 7px", fontSize: 10, fontWeight: 800 }}>{cart.reduce((s, i) => s + i.qty, 0)}</span>
            </button>
          )}
        </div>
      </div>
      ...
      {products.length === 0 ? (
        isAdmin && (
          <div style={{ textAlign: "center", padding: "30px 20px", background: T.bgCard, border: `1px dashed ${T.border}`, borderRadius: 16 }}>
            <div style={{ fontSize: 36, marginBottom: 10 }}>🖨️</div>
            <div style={{ color: T.textMid, fontSize: 13, marginBottom: 14 }}>No products listed yet</div>
            <button onClick={() => setShowAdd(true)} style={{ background: "linear-gradient(135deg,#f97316,#ea6008)", border: "none", borderRadius: 10, padding: "10px 20px", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ Add First Product</button>
          </div>
        )
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 14 }}>
          {products.map(p => (
            <ProductCard key={p.id} product={p} isAdmin={isAdmin} onAddCart={addToCart} onEdit={setEditProduct} />
          ))}
        </div>
      )}
